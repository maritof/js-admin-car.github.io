...
async function getCarList(url) {
  const resp = await fetch(url + /cars-list/, {
    method: "GET",
  });
  const carList = await resp.json();
  return carList;
}

function showCars(carList) {
  const table = document.getElementById("table_car_list");
  let tbody = "<tbody>";
  for (let i = 0; i < carList.length; i++) {
    ...
    <td>
    <a onclick='sendParams(${carList[i].id})' title="Editar">
     <svg
     version="1.1"
   ...
   >
     ...
     <a onclick='deleteCar(${carList[i].id})' title="Eliminar">
     <svg
       height="512pt"
...
async function sendParams(id) {
  window.location.href = `car.html?car=${id}`;
}

async function deleteCar(id) {
  if (confirm("Esta seguro de eliminar? No podra revertir esto!")) {
    await fetch(url + /cars-delete/ + id + "/", {
      method: "DELETE",
    });
  }
  else {
